...
pub struct ChatOptions {
	...
	/// Note: These headers will be added to the ones generated by the adapter.
...
impl ChatOptions {
	...
	/// Add a custom HTTP header to be sent with the request.
...
impl ChatOptionsSet<'_, '_> {
	...
	pub fn headers(&self) -> Option<Vec<(String, String)>> {
		...
		if let Some(client_headers) = self.client.and_then(|c| c.headers.as_ref()) {
			headers.extend(client_headers.clone());
		}

		// chat headers second (overwrite or append? usually append for headers, but here we probably want to support both)
		if let Some(chat_headers) = self.chat.and_then(|c| c.headers.as_ref()) {
			headers.extend(chat_headers.clone());
		}
